...
if test -f "dsa_annotations/heatmap/oncofusion-tissue_${sample_name}.json"; then
	echo "dsa_annotations/heatmap/oncofusion-tissue_${sample_name}.json exists"
else
	sed '1s/score_0/Stroma/g; 1s/score_1/Tumor/; 1s/score_2/Fat/; 1s/score_3/Necrosis/' inference/tissue_type_classifier_weights/${sample_name}.csv > inference/tissue_type_classifier_weights/${sample_name}_renamed.csv
	perl -MEnv -F, -lane 'chop; if ($. == 1) { print $_ . ",coordinates" } else { ($x, $y) = $F[0] =~ /(\d+)_(\d+)\.png/; print $_ . "," . "\"($x,$y)\"";}' inference/tissue_type_classifier_weights/${sample_name}_renamed.csv > inference/tissue_type_classifier_weights/${sample_name}_renamed_coord.csv
	dsa heatmap inference/tissue_type_classifier_weights/${sample_name}_renamed_coord.csv \
	    --annotation_name oncofusion-tissue \
	    --image_filename $filename \
...
if test -f "dsa_annotations/stardist_polygon/oncofusion-cell_${sample_name}.json"; then
	echo "dsa_annotations/stardist_polygon/oncofusion-cell_${sample_name}.json exists"
else
